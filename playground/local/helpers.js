...
export const generateSources = sources => {
  return getUniqSources(sources).join(',');
};
...
export const generateClusterArticles = (articles, outputConfig) => {
  const result = articles.map(article => {
    ...
    articleContent.push({
      text: `${moment(article.published_at).format(
        'MMMM DD, YYYY'
      )} | ${generateSources(article.sources)}`,
      fontStyle: 'light',
      fontSize: 8,
      align: 'left',
      marginLeft: 40,
...
export const generateClusterContent = (cluster, outputConfig, tocIds = []) => {
  ...
  if (outputConfig.summary_bullets > 0) {
    ...
    clusterContent = clusterContent.concat(
  ...
  clusterContent.push(
    generateClusterArticles(cluster.fetchedArticles, outputConfig)
  );
